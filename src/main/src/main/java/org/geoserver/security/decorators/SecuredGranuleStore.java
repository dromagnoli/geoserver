...
class SecuredGranuleStore extends SecuredGranuleSource implements GranuleStore {
    ...
    /**
     * Granule collection failing the iteration as soon as a granule the read filter excludes shows up. A spatial read
     * filter is compared without reprojection, so its geometries must be in the granule CRS, as the catalog side does.
     */
