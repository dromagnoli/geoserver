...
import org.geoserver.web.wicket.browser.CogInput;

public class CogFileParamPanel extends Panel implements ParamPanel {
    CogInput cogInput;
    ...
    public CogFileParamPanel(
        ...
        add(label);
        this.cogInput = getFilePathInput(paramValue, paramLabelModel, required, validators);
        add(cogInput);
    }

    protected CogInput getFilePathInput(
            IModel<String> paramValue,
            IModel<String> paramLabelModel,
            boolean required,
            IValidator<? super String>[] validators) {
        // the file chooser
        return new CogInput("cogInput", paramValue, paramLabelModel, required, validators);
    }
