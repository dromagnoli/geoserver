...
public abstract class AbstractNetCDFEncoder implements NetCDFEncoder {
    ...
    @SuppressWarnings(
            "deprecation") // No valid alternative. Even the FormatWriter uses that class too
    ...
    protected NetcdfFormatWriter.Builder getWriterBuilder(File file, String outputFormat)
        ...
        NetcdfFormatWriter.Builder writer = null;
        if (ncFormat == NetcdfFileFormat.NETCDF4_CLASSIC) {
            ...
            writer = NetcdfFormatWriter.createNewNetcdf4(ncFormat, file.getAbsolutePath(), chunker);
        }
    ...
    public void write() throws IOException, ucar.ma2.InvalidRangeException {
        try {
            // The writer has been built, let's pass it to the CRSWriter
            crsWriter.setWriter(writer);
