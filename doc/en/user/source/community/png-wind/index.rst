...
A heuristic is used to determine whether the two bands represent U/V or Speed/Direction based on a configuration file.
...
Alternatively, the configured default min/max values will be used.
...
The band name used in the matching process is taken from the coverage metadata.
If necessary, it can be customized in the Coverage Band Details section of the layer configuration in GeoServer.
...
The coverage must contain exactly two bands representing the wind components either 
