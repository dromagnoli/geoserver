...
Let's add this ndvi.sld style to apply a proper colormap to the NDVI band:

.. include:: src/modisvi/ndvi.sld
   :literal:
...
.. admonition:: curl request to create the style
   ...
   ::
       ...
       curl -v -u admin:geoserver -XPOST -H "Content-type: text/xml" -d "<style><name>ndvi</name><filename>ndvi.sld</filename></style>" http://localhost:8080/geoserver/rest/styles

.. admonition:: curl request to upload the content of the style

   ::

	   curl -v -u admin:geoserver -XPUT -H "Content-type: application/vnd.ogc.sld+xml" -d @ndvi.sld http://localhost:8080/geoserver/rest/styles/ndvi
